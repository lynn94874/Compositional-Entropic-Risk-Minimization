...
class tpAUC_CVaR_loss(torch.nn.Module):
    ...
    r"""
        ...
        Args:
            ...
            beta (float, optional): step size for updating s1 (default: ``1e-1``).
            beta_ (float, optional): step size for updating s2 (default: ``1e-1``).
        ...
        Example:
            >>> loss_fn = tpAUC_CVaR_loss(data_length=1000, rate=0.5)
            >>> y_pred = torch.randn(32, 1, requires_grad=True)
            >>> y_true = torch.randint(0, 2, (32,))
            >>> ids_p = torch.arange(16)  # indices of positive samples
            >>> loss = loss_fn(y_pred, y_true, ids_p)
            >>> loss.backward()

    """
    def __init__(self, data_length, threshold=0.5, alpha=1e-1, beta=1e-1, beta_=1e-1, theta_0=0.5, theta_1=0.5, device=None):
        ...
        self.dual_var = torch.tensor([0.0]*data_length).view(-1, 1).to(self.device)
        self.threshold = threshold

    def forward(self, y_pred, y_true, ids_p): 
        ...
        self.dual_var[ids_p] = torch.clamp(self.dual_var[ids_p] + self.eta_dual * (self.u[ids_p]-self.s2)/self.theta_0,
                                                        min=0.0,
                                                        max=1)
        
        weighted_loss = torch.mean(self.dual_var[ids_p] * tp_loss)

        self.s1[ids_p] -= self.eta_1*(self.dual_var[ids_p]/len(ids_p)/self.theta_0*(1-p1.mean(dim=-1, keepdim=True)/self.theta_0))
        self.s2 -= self.eta_2 * (1-torch.mean(self.dual_var[ids_p])/self.theta_0)
